...
public class VillagerBeaconEntityRenderer implements BlockEntityRenderer<VillagerBeaconEntity> {
    public static final Identifier BEAM_TEXTURE = new Identifier(KNB.modName, "textures/entity/nether_beam.png");

    public VillagerBeaconEntityRenderer(BlockEntityRendererFactory.Context ctx) {
    }

    public void render(VillagerBeaconEntity beaconEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        long l = beaconEntity.getWorld().getTime();
        List<VillagerBeaconEntity.BeamSegment> list = beaconEntity.getBeamSegments();
        int k = 0;

        for(int m = 0; m < list.size(); ++m) {
            VillagerBeaconEntity.BeamSegment beamSegment = list.get(m);
    ...
    public boolean rendersOutsideBoundingBox(VillagerBeaconEntity beaconEntity) {
        return true;
    }
    ...
    public boolean method_33892(VillagerBeaconEntity beaconBlockEntity, Vec3d vec3d) {
