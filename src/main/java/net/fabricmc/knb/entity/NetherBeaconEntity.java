...
public class NetherBeaconEntity extends BlockEntity implements NamedScreenHandlerFactory {
    ...
    private static StatusEffect[] MOB_EFFECTS = new StatusEffect[]{StatusEffects.GLOWING, StatusEffects.INSTANT_DAMAGE, StatusEffects.WITHER, StatusEffects.LEVITATION};
    private static StatusEffect[] PLAYER_EFFECTS = new StatusEffect[]{StatusEffects.INVISIBILITY, StatusEffects.WITHER, StatusEffects.LEVITATION, StatusEffects.FIRE_RESISTANCE, StatusEffects.SATURATION, StatusEffects.WATER_BREATHING};
    ...
    private static void applyPlayerEffects(World world, BlockPos pos, int beaconLevel, @Nullable StatusEffect primaryEffect, @Nullable StatusEffect secondaryEffect) {
        if (!world.isClient) {
            ...
            if (secondaryEffect != null) {
                // against mobs
                if (true || Arrays.stream(MOB_EFFECTS).toList().contains(secondaryEffect)) {
                ...
                // against players
                if (true || Arrays.stream(PLAYER_EFFECTS).toList().contains(secondaryEffect)) {
                    List<PlayerEntity> list2 = world.getNonSpectatingEntities(PlayerEntity.class, box);
                    Iterator var22 = list2.iterator();
                    PlayerEntity p;
                    while(var22.hasNext()) {
                        p = (PlayerEntity)var22.next();
                        p.addStatusEffect(new StatusEffectInstance(secondaryEffect, j, 0, true, true));
                    }
                }
            }
        }
    ...
    static {
        ...
        EFFECTS_BY_LEVEL = new StatusEffect[][]{{StatusEffects.GLOWING, StatusEffects.INSTANT_DAMAGE}, {StatusEffects.INVISIBILITY, StatusEffects.WITHER},
                {StatusEffects.LEVITATION, StatusEffects.SATURATION, StatusEffects.WATER_BREATHING}, {StatusEffects.FIRE_RESISTANCE}};
        // EFFECTS_BY_LEVEL = new StatusEffect[][]{{StatusEffects.SPEED, StatusEffects.HASTE}, {StatusEffects.RESISTANCE, StatusEffects.JUMP_BOOST}, {StatusEffects.STRENGTH}, {StatusEffects.REGENERATION}};
        // k swap out effects
